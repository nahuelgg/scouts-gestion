...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    ...
    const uploadPath = path.join(process.env.UPLOAD_PATH || './uploads', year.toString());
  ...
  try {
    ...
    if (!socio || !monto || !mesCorrespondiente || !metodoPago) {
      return res.status(400).json({ 
        message: 'Socio, monto, mes correspondiente y método de pago son requeridos' 
      });
    }
  ...
  try {
    ...
    if (req.file) {
      ...
      if (pago.comprobante && pago.comprobante.path) {
        const oldFilePath = path.join(process.env.UPLOAD_PATH || './uploads', pago.comprobante.path);
...
const deletePago = async (req, res) => {
  try {
    ...
    if (pago.comprobante && pago.comprobante.path) {
      const filePath = path.join(process.env.UPLOAD_PATH || './uploads', pago.comprobante.path);
