...
router.route('/')
  ...
  .post(protect, authorize('jefe_de_rama', 'administrador'), upload.single('comprobante'), createPago);

router.route('/:id')
  .get(protect, getPagoById)
  .put(protect, authorize('jefe_de_rama', 'administrador'), upload.single('comprobante'), updatePago)
