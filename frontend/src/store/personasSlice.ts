...
export const fetchPersonas = createAsyncThunk(
  ...
  async (params: any = {}, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || 'Error obteniendo personas');
    }
  }
);
...
export const fetchPersonaById = createAsyncThunk(
  ...
  async (id: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || 'Error obteniendo persona');
    }
  }
);
...
export const createPersona = createAsyncThunk(
  ...
  async (data: PersonaFormData, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || 'Error creando persona');
    }
  }
);
...
export const updatePersona = createAsyncThunk(
  ...
  async ({ id, data }: { id: string; data: PersonaFormData }, { rejectWithValue }) => {
    try {
      const response = await personasAPI.update(id, data);
      return response;
    } catch (error: any) {
      return rejectWithValue(error.response?.data?.message || 'Error actualizando persona');
    }
  }
);
...
export const deletePersona = createAsyncThunk(
  ...
  async (id: string, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.message || 'Error eliminando persona');
    }
  }
);
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(updatePersona.fulfilled, (state, action) => {
        state.isLoading = false;
        const index = state.personas.findIndex(p => p._id === action.payload.persona._id);
        if (index !== -1) {
